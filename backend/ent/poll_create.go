...
func (pc *PollCreate) SetVotenums(m map[string]int) *PollCreate {
	pc.mutation.SetVotenums(m)
	return pc
}
...
func (pc *PollCreate) check() error {
	...
	if _, ok := pc.mutation.Options(); !ok {
		return &ValidationError{Name: "options", err: errors.New(`ent: missing required field "Poll.options"`)}
	}
	if _, ok := pc.mutation.Votenums(); !ok {
		return &ValidationError{Name: "votenums", err: errors.New(`ent: missing required field "Poll.votenums"`)}
	}
...
func (pc *PollCreate) createSpec() (*Poll, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.Votenums(); ok {
		_spec.SetField(poll.FieldVotenums, field.TypeJSON, value)
		_node.Votenums = value
	}
